...
if (!fs.existsSync(configFile)) {
  ThrowDevError('tailwind.config.js file is required!')
}

const { theme } = require(configFile)
const util = {}
...
const validateSpacingVals = (val) => {
  ...
  if (!spacingKeys.includes(val) || !val) {
    ThrowDevError(`${val} doesn't exist in theme.spacing. See: tailwind config`)
  }
}
...
const margin = (val, side) => {
  ...
  switch (side) {
    case 'y':
      return { marginTop: theme.spacing[val], marginBottom: theme.spacing[val] }
    case 'x':
      return { marginLeft: theme.spacing[val], marginRight: theme.spacing[val] }
    case 't':
...
util.font = (val) => {
  ...
  if (typeof val !== 'string') {
    ...
    ThrowDevError(
      `${val} doesn't exist in fontWeight or fontFamily. See: tailwind config`
    )
  }
}
...
util.text = (val) => {
  ...
  if (typeof val !== 'string') {
    ...
    ThrowDevError(
      `${val} doesn't exist in fontSize or colors. See: tailwind config`
    )
  }
}
...
util.bg = (val) => {
  ...
  if (colorKeys.includes(val)) {
    ...
    ThrowDevError(`${val} doesn't exist in colors. See: tailwind config`)
  }
}
...
util.rounded = (val) => {
  ...
  if (parts.length === 1) {
    if (!borderRadiusKeys.includes(val)) {
      ThrowDevError(
        `${val} doesn't exist in borderRadius. See: tailwind config`
      )
    }
  ...
  if (!borderRadiusKeys.includes(parts[1])) {
    ThrowDevError(
      `${parts[1]} doesn't exist in borderRadius. See: tailwind config`
    )
  }
...
util.opacity = (val) => {
  ...
  ThrowDevError(`${val} doesn't exist in opacity. See: tailwind config`)
}
...
util.shadow = (val) => {
  ...
  ThrowDevError(`${val} doesn't exist in boxShadow. See: tailwind config`)
}
...
util.elevation = (val) => {
  ...
  ThrowDevError(`${val} doesn't exist in elevation. See: tailwind config`)
}
